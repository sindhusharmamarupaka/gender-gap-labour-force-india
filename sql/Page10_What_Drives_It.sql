-- ============================================================================
-- FILE:  Page10_What_Drives_It.sql
-- Gender Gap in Labour Force Participation - India -- Page 10: "What Drives It?"
...
-- ============================================================================
-- PAGE 10 -- "What Drives It?" (Key Influencers AI visual)  (2 analyses)
-- ============================================================================
-- The Key Influencers visual runs a multivariate ML model (logistic-
-- regression-style influence scoring) internally - that ranking cannot be
-- reproduced in plain SQL. What CAN be validated is the single-factor rate
-- behind the visual's headline claim ("Currently Married = 2.51x").
-- Target field = Employment_Status_Code = 92 (Domestic Duties Only).

-- P10.1 -- Domestic Duty Rate (Female) by Marital Status - validates the
-- "Currently Married = 2.51x" headline claim as a simple rate-ratio.
-- [NOTE: recomputed this session gives Married 44.69% vs Never-Married
-- 13.76% = ~3.25x by simple ratio - the AI's 2.51x is a *multivariate*
-- influence weight, not a raw ratio, so an exact match isn't expected;
-- this confirms the DIRECTION and rough MAGNITUDE only, not the exact 2.51.]
SELECT
    ...
    ROUND(SUM(CASE WHEN Employment_Status_Code = 92 THEN 1 ELSE 0 END) / COUNT(*) * 100, 2) AS Domestic_Duty_Rate
FROM plfs_working_age
WHERE Gender = 2
GROUP BY Marital_Status
ORDER BY Domestic_Duty_Rate DESC;

-- P10.2 -- Domestic Duty Rate (Female) by the other three Explain-By fields
-- used in the Key Influencers visual: Sector, Education Level, Age band
SELECT
    CASE Sector_Rural_Urban WHEN 1 THEN 'Rural' WHEN 2 THEN 'Urban' END AS Sector,
    General_Education_Level,
    CASE WHEN Age < 25 THEN '<25' WHEN Age BETWEEN 25 AND 44 THEN '25-44'
         WHEN Age BETWEEN 45 AND 64 THEN '45-64' ELSE '65+' END AS Age_Band,
    ROUND(SUM(CASE WHEN Employment_Status_Code = 92 THEN 1 ELSE 0 END) / COUNT(*) * 100, 2) AS Domestic_Duty_Rate,
    COUNT(*) AS N
FROM plfs_working_age
WHERE Gender = 2
GROUP BY Sector, General_Education_Level, Age_Band
ORDER BY Domestic_Duty_Rate DESC;
